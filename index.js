...
const log = Object.assign((...args) => console.log('[gemini]', ...args), {
  warn: (...args) => console.warn('[gemini]', ...args),
  error: (...args) => console.error('[gemini]', ...args)
})
...
export default function createGeminiPlugin(_opts = {}) {
  ...
  const plugin = {
    ...
    onConsoleInput: async ({ text, client, sendJsonMode }) => {
      await plugin.onMessage({
        event: { text: `${trigger}${text}` },
        client,
        sendJsonMode
      })
    },
...
export const metadata = {
  name: 'gemini',
  description:
    'Gemini AI plugin for meshtastic-node-client. Listens for messages starting with TRIGGER_TEXT (default "G, "), sends the remainder to the Google Gemini API, and broadcasts the reply to the channel. Required env: GEMINI_API_KEY. Optional env: GEMINI_TRIGGER_TEXT, GEMINI_MODEL, GEMINI_MAX_LENGTH, GEMINI_CHUNK_SIZE, GEMINI_USE_WS.'
}

export { splitMessage, GeminiLiveClient }
